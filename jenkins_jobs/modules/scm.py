...
def git_extensions(xml_parent, data):
    ...
    if 'clean' in data:
        ...
        if clean_after:
            ...
            if trait:
                tr = XML.SubElement(xml_parent, trait_prefix + 'CleanAfterCheckoutTrait')
        ...
        if clean_before:
            ...
            if trait:
                tr = XML.SubElement(xml_parent, trait_prefix + 'CleanBeforeCheckoutTrait')
                ext = XML.SubElement(tr, "extension", {"class": ext_name})
            else:
                ext = XML.SubElement(xml_parent, ext_name)
    ...
    if 'submodule' in data:
        ...
        if trait:
            tr = XML.SubElement(xml_parent, trait_prefix + 'SubmoduleOptionTrait')
            ext = XML.SubElement(tr, "extension", {"class": ext_name}) 
        else:
            ext = XML.SubElement(xml_parent, ext_name)
    ...
    if 'timeout' in data:
        ...
        if trait:
            tr = XML.SubElement(xml_parent, trait_prefix + 'CheckoutOptionTrait')
            ext = XML.SubElement(tr, "extension", {"class": ext_name}) 
        else:
            ext = XML.SubElement(xml_parent, ext_name)
        XML.SubElement(ext, 'timeout').text = str(data['timeout'])
    ...
    if prune == 'true':
        ...
        if trait:
            tr = XML.SubElement(xml_parent, trait_prefix + 'PruneStaleBranchTrait')
            ext = XML.SubElement(tr, "extension", {"class": ext_name}) 
        else:
            ext = XML.SubElement(xml_parent, ext_name)
    ...
    if wipe_workspace == 'true':
        ...
        if trait:
            tr = XML.SubElement(xml_parent, trait_prefix + 'WipeWorkspaceTrait')
            ext = XML.SubElement(tr, "extension", {"class": ext_name}) 
        else:
            ext = XML.SubElement(xml_parent, ext_name)
    ...
    if use_author == 'true':
        ...
        if trait:
            tr = XML.SubElement(xml_parent, trait_prefix + 'AuthorInChangelogTrait')
            ext = XML.SubElement(tr, "extension", {"class": ext_name}) 
        else:
            ext = XML.SubElement(xml_parent, ext_name)
